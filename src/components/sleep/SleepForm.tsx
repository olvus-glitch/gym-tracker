...
export default function SleepForm({ date, onGuardar }: Props) {
  ...
  return (
    <div className="bg-card-bg rounded-xl border border-border p-4 space-y-3">
      <h3 className="text-sm font-bold text-foreground">Registro de sueño</h3>
      <div>
      ...
      <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Notas de sueño (opcional)" rows={2} className="w-full px-3 py-2 rounded-lg border border-border bg-input-bg text-sm" />
      <button
